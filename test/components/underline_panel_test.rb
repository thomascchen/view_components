...
class PrimerUnderlinePanelComponentTest < Minitest::Test
  include Primer::ComponentTestHelpers

  def test_renders_panels_and_tab_container
    render_inline(Primer::UnderlinePanel.new(label: "label")) do |component|
  ...
  def test_customizes_tab_container
    render_inline(Primer::UnderlinePanel.new(label: "label", with_panel: true, wrapper_arguments: { m: 2, classes: "custom-class" })) do |component|
      component.tab(selected: true, id: "tab-1", panel_id: "panel-1") do |t|
        t.panel { "Panel 1" }
        t.text { "Tab 1" }
      end
  ...
  def test_raises_if_multiple_tabs_are_selected
    err = assert_raises Primer::TabbedComponentHelper::MultipleSelectedTabsError do
      render_inline(Primer::UnderlinePanel.new(label: "label")) do |c|
        ...
        c.tab(selected: true, id: "tab-3", panel_id: "panel-3") do
          t.panel { "Panel 3" }
          t.text { "Tab 3" }
        end
      end
  ...
  def test_align_falls_back_to_default
    without_fetch_or_fallback_raises do
      render_inline(Primer::UnderlinePanel.new(label: "label", align: :foo)) do |component|
        component.tab(selected: true, id: "tab-1", panel_id: "panel-1") do |t|
          t.text { "Tab 1" }
          t.panel { "Panel 2" }
        end
  ...
  def test_adds_underline_nav_right_when_align_right_is_set
    render_inline(Primer::UnderlinePanel.new(label: "label", align: :right)) do |component|
      component.tab(selected: true, id: "tab-1", panel_id: "panel-1") do |t|
        t.text { "Tab 1" }
        t.panel { "Panel 2" }
      end
  ...
  def test_renders_tab_icon_with_correct_classes
    render_inline(Primer::UnderlinePanel.new(label: "label", align: :right)) do |component|
      component.tab(selected: true, id: "tab-1", panel_id: "tab-1=") do |t|
        t.text { "Tab 1" }
        t.panel { "Panel 1" }
        t.icon(icon: :star)
      end
